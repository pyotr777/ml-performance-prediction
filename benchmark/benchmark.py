...
parser.add_argument('--testVGG16', action="store_true", default=False,
                    help='Benchmark training VGG-16 on sythetic data')

# General parameters
parser.add_argument('--saveto', type=str, default='/results', help='Path to save benchmarks CSV files')
...
parser.add_argument('--use_tf_profiler', action="store_true", default=False,
                    help='Calculate number of FLOPs with ft profiler')
...
parser.add_argument('--logFLOPs', type=int, default=0,
                    help='log10 of number of FLOPs to perform (will be rounded up), overwrites iter_benchmark, set 0 to use iter')
...
def main(_):
    ...
    if args.testMatMul:
        if args.logfile == '':
            logfile = os.path.join(RES_DIR, str('benchmark_matmul_%s_%s'
    ...
    if args.testConv:
        if args.logfile == '':
            logfile = os.path.join(RES_DIR, str('benchmark_convolution_%s_%s'
                                                % (args.device, time.strftime("%Y%m%d"))))
        else:
            logfile = args.logfile
        print("Save logfile to {}".format(logfile))
    ...
    if args.testVGG16:
        if args.logfile == '':
            logfile = os.path.join(RES_DIR, str('benchmark_VGG16_%s_%s'
                                                % (args.device, time.strftime("%Y%m%d"))))
        else:
            logfile = args.logfile
        print("Save logfile to {}".format(logfile))
